...
| [`code-samples/02-write-block-dispatch.cs`](code-samples/02-write-block-dispatch.cs) | Doc-action separation applied immediately before dispatch, and the refusal that goes back for a blocked call. The latch is set through `AddRetrieved`, so `state.HasRetrieved = true` does not compile, and whether a call is a write is looked up in the catalog rather than carried on the call. |
